...
####################################### widget adjust###################################
    ...
    def filter_thresh_2area_adjust(self,region1,region2):
        ...
        thresh1=cv2.resize(edges1,(80,50))

        h_s1,w_s1=thresh1.shape
        step_s1= w_s1
        q1 = QImage(bytes(thresh1.data),w_s1,h_s1,step_s1, QImage.Format_Grayscale8)
        self.ui.space_s1.setPixmap(QPixmap.fromImage(q1))

        thresh2=cv2.resize(edges2,(80,50))

        h_s2,w_s2=thresh2.shape
        step_s2= w_s2
        q2 = QImage(bytes(thresh2.data),w_s2,h_s2,step_s2, QImage.Format_Grayscale8)
        self.ui.space_s2.setPixmap(QPixmap.fromImage(q2))
        ...
        self.ui.value_s1_area.setText(str(largest_area1))
            

        if contours2:
            largest_contour2 = max(contours2, key=cv2.contourArea)
            largest_area2 = int(cv2.contourArea(largest_contour2))
        self.ui.value_s2_area.setText(str(largest_area2))
    ...
    def check_push_aid_adjust(self,image):
        count_aid=0
        bit_set=False
        ...
        upper_hsv=np.array([179, 255, self.value_thresh_aid])
        ...
        edges = cv2.erode(mask, kernel11, iterations=1)
        edges = cv2.dilate(edges, kernel11, iterations=k)

        edges_show=edges
        edges_show=cv2.resize(edges_show,(140,140))
        h,w=edges_show.shape
        step= w
        q = QImage(edges_show.data,w,h,step, QImage.Format_Grayscale8)
        self.ui.space_image_aid_thread.setPixmap(QPixmap.fromImage(q))

        contours, _ = cv2.findContours(edges, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        if contours:
            largest_contour = max(contours, key=cv2.contourArea)
            largest_area1 = int(cv2.contourArea(largest_contour))
            self.ui.value_area_aid_show.setText(str(largest_area1))
            ...
            if largest_area1< self.value_area_thresh_aid_limit:
                for contour in contours:
                    ...
                    if cv2.contourArea(contour) > self.value_area_thresh_aid_min and cv2.contourArea(contour)< self.value_area_thresh_aid_noise or  cv2.contourArea(contour) > self.value_area_thresh_aid_max and cv2.contourArea(contour)< self.value_area_thresh_aid_limit:
                        count_aid+=1
                self.ui.value_count_aid.setText(str(count_aid))

                contour_mask_show=cv2.resize(contour_mask,(140,140))
                h,w=contour_mask_show.shape
                step= w
                q = QImage(contour_mask_show.data,w,h,step, QImage.Format_Grayscale8)
                self.ui.space_image_aid.setPixmap(QPixmap.fromImage(q))

                contours_mask, _ = cv2.findContours(contour_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                if contours_mask:
                    if bit_set==False and count_aid==1:
                        self.ui.label_aid_ok.show()
                        for contour in contours_mask:
                            cv2.drawContours(image_clr, [contour], -1, (0, 0, 255), 2) #### draw ok
                        return True,image_clr
                    else:
                        self.ui.label_aid_ok.hide()            
                        return False,image_clr
                else:
                    self.ui.label_aid_ok.hide()            
                    return False,image_clr
            else:
                # self.ui.space_image_aid.clear()
                self.ui.label_aid_ok.hide()            
                return False,image_clr
        else:
            # self.ui.space_image_aid.clear()
    ...
    def check_screw_adjust(self,image):
        count_area=0
        count_number=0
        ...
        # _, thresh = cv2.threshold(image, self.value_thresh_screw, 255, cv2.THRESH_BINARY)
        lower_hsv=np.array([0,0,0])
        ...
        # edges1_show=edges1
        edges1_show=cv2.resize(mask,(140,140))
        h,w=edges1_show.shape
        step= w
        q = QImage(edges1_show.data,w,h,step, QImage.Format_Grayscale8)
        self.ui.space_image_screw_thread.setPixmap(QPixmap.fromImage(q))
        
        contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        if contours:
            for contour in contours:
                if cv2.contourArea(contour) > self.value_area_thresh_screw_min and cv2.contourArea(contour) < self.value_area_thresh_screw_max:
                    # count_area=count_area+cv2.contourArea(contour)
                    # Vẽ các contour lên ảnh trống
                    count_number+=1
                    cv2.drawContours(contour_mask, [contour], -1, 255, thickness=cv2.FILLED)

            self.ui.value_result_count_screw.setText(str(count_number))

            contour_mask_show=cv2.resize(contour_mask,(140,140))
            h,w=contour_mask_show.shape
            step= w
            q = QImage(contour_mask_show.data,w,h,step, QImage.Format_Grayscale8)
            self.ui.space_image_screw.setPixmap(QPixmap.fromImage(q))
    ...
    def check_push_pin_adjust(self,image):
        ...
        edges1_show=edges
        edges1_show=cv2.resize(edges1_show,(140,140))
        h,w=edges1_show.shape
        step= w
        q = QImage(edges1_show.data,w,h,step, QImage.Format_Grayscale8)
        self.ui.space_image_pin_thread.setPixmap(QPixmap.fromImage(q))
        ...
        if contours:
            ...
            largest_area = int(cv2.contourArea(largest_contour))
            for contour in contours:
                if cv2.contourArea(contour) > self.value_area_thresh_pin_min and cv2.contourArea(contour) < self.value_area_thresh_pin_max:
                    if cv2.contourArea(contour)> self.value_double_pin:
                        count+=2
                    else:
                        count+=1
                        
                    cv2.drawContours(contour_mask, [contour], -1, 255, thickness=cv2.FILLED)
        # white_pixel_count = np.sum(contour_mask == 255)
        # print(count)
        self.ui.value_result_count_pin.setText(str(count))
        self.ui.value_area_double_pin_show.setText(str(largest_area))
        # self.ui.value_result_area_screw.setText(str(white_pixel_count))

        contour_mask_show=cv2.resize(contour_mask,(140,140))
        h,w=contour_mask_show.shape
        step= w
        q = QImage(contour_mask_show.data,w,h,step, QImage.Format_Grayscale8)
        self.ui.space_image_pin.setPixmap(QPixmap.fromImage(q))
...
#########################################################################################
    ...
    def show_img_NG(self):
        ...
        if item_img_ng is not None:
            ...
            # print(link_NG_picture)
            #print(item_show_ng_text)
            self.ui.space_to_display_history_NG.setPixmap(QtGui.QPixmap(os.path.join(self.path_folder_today, link_NG_picture)))
    ...
    def conveyor_running_count(self,frame_gray):
        ...
        if self.banana>1 :
            ...
            if (self.banana-self.banana_ate>12): ## nếu quá 5 khung hình mà không nhận được rằng có sự chuyển động thì xử lý 
                ...
                if self.count_times>self.over_point: # khi vượt qua điểm giới hạn thì kích hoạt việc xoay hình 
                    if self.mode_auto_scan==False:
                        ...
                        self.ui.count_sum.setText(' Sum of products: {}'.format(self.count_sum))
                        image_to_rotate=frame_gray.copy()[:218,:]
                        ...
                        h,w=image_to_rotate.shape
                        step= w
                        q = QImage(image_to_rotate.data,w,h, step, QImage.Format_Grayscale8)
    ...
    def rotate_image(self,image): ### để xoay dựa vào sponge
        ...
        image=cv2.resize(image,(320,250))
        blurred = cv2.GaussianBlur(image.copy(), (11, 11), 1)
        _, thresh = cv2.threshold(blurred, self.value_thresh, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)# tham số ngưỡng phán định #self.value_thresh
        ...
        # if contours:
            ...
            if largest_area>self.value_area_thresh and largest_area<25000:## self.value_area_thresh
                rect = cv2.minAreaRect(largest_contour)
                box = cv2.boxPoints(rect)
                box = np.intp(box)
    ...
    while True:
        ...
        # try:
            if grabResult.GrabSucceeded():
                ...
                image_grabed_resize=image_grabed_resize[:,110:280]
